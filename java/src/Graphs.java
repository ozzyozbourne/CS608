...
public final class Graphs {
    ...
    public static void main(final String... args) {
        for (final int vertexCount : new int[] { 10, 100, 1000 }) {
            for (final Function<Integer, Integer> density : densities) {
                ...
                final Map<Integer, List<Integer>> graph = generateDirectedGraph(
                    vertexCount,
                    edgeCount
                );
                ...
                for (final int vertex : graph.keySet()) {
                    if (!visited.contains(vertex)) dfsVisit(graph, vertex);
                }
                final long endTime = System.nanoTime();
                final int travelTime =
                    Collections.max(finishTimes.values()) - 1; // start time is always one
                //
                System.out.printf("\nDiscovery Times -> \n");
                discoveryTimes
                    .entrySet()
    ...
    private static void dfsVisit(
        ...
        for (final int neighbor : graph.get(vertex)) {
            if (!visited.contains(neighbor)) dfsVisit(graph, neighbor);
        }
    ...
    public static Map<Integer, List<Integer>> generateDirectedGraph(
        ...
        for (int i = 0; i < vertexCount; i++) adjacencyList.put(
            i,
            new ArrayList<>()
        );
